...
function buildSourceSection(sourcePaths: string[], config: Config): string {
  const fileSources = sourcePaths.length > 0
    ...
    : '';
  // Combine URL sources from both privateInfo and publicInfo for generation
  const allUrls = [
...
function buildPrompt(sourcePaths: string[], config: Config, existingTests: TestCase[] = []): string {
  ...
  const docsUrl = publicUrls[0]?.url ?? '';
...
${pkg?.language ? `\nIMPORTANT: All test cases and reference solutions MUST use ${pkg.language}.\n` : ''}${buildTargetContext(config)}${SCHEMA_DESCRIPTION}`;
}
...
export async function generateCommand(paths: ProjectPaths, options: { fresh?: boolean; nonInteractive?: boolean; promptOnly?: boolean } = {}): Promise<void> {
