...
import { getPackageSource, getUrlSources } from '../types.js';
...
Per-test-case files (in {resultsDir}/{target}/{testId}/):
  ...
  agent-egress.log.json        — executor egress traffic logs
  agent-session.jsonl          — agent session/conversation log (if available)
  ...
  judge-egress.log.json        — judge egress traffic logs
...
export function buildInsightsPrompt(
  ...
  // 2. Source locations — combine URLs from both privateInfo and publicInfo
  const allUrls = [
    ...getUrlSources(config.privateInfo),
    ...getUrlSources(config.publicInfo ?? []),
  ];
  const urlSection = buildUrlSourceList(allUrls.map(s => ({ url: s.url, additionalContext: s.additionalContext })));
...
export async function insightsCommand(paths: ProjectPaths, options: { fresh?: boolean; promptOnly?: boolean } = {}): Promise<void> {
  const config = await loadConfig(paths.config);

  {
    const spinner = options.promptOnly ? null : ora('Resolving sources...').start();
    const sourcePaths = await resolveSources(config.privateInfo, { fresh: options.fresh, reposDir: paths.cacheRepos });
    spinner?.succeed(`Sources resolved: ${sourcePaths.join(', ')}`);

    const loadSpinner = options.promptOnly ? null : ora('Loading test suite and results...').start();
    ...
    if (totalResults === 0) {
      if (options.promptOnly) {
        process.stderr.write('No results found. Run the pipeline first: agentic-usability eval\n');
        process.exit(1);
      }
