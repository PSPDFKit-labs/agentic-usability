...
describe('generateCommand (prompt-only)', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.spyOn(console, 'log').mockImplementation(() => {});
  ...
  it('outputs prompt to stdout without spawning an agent', async () => {
    await generateCommand(paths, { promptOnly: true });

    expect(process.stdout.write).toHaveBeenCalledWith(expect.stringContaining('test case generator'));
    expect(createAdapter).not.toHaveBeenCalled();
  });

  it('includes suite file path in the prompt', async () => {
    await generateCommand(paths, { promptOnly: true });

    const output = vi.mocked(process.stdout.write).mock.calls[0][0] as string;
    expect(output).toContain('suite.json');
  });
});
