...
export function buildSourceList(sourcePaths: string[], config: Config): string {
  // sourcePaths only contains local/git sources, so match them to file-resolvable config entries
  const fileSources = config.privateInfo.filter((s) => s.type === 'local' || s.type === 'git');
  return sourcePaths.map((p, i) => {
    const ctx = fileSources[i]?.additionalContext;
    return ctx ? `- ${p} — ${ctx}` : `- ${p}`;
  }).join('\n');
}
