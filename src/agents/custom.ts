...
export class CustomAdapter extends BaseAdapter {
  ...
  readonly baseUrlEnvVar = null;
  readonly defaultEnvVar = null;
  readonly defaultBaseUrl = null;
  ...
  sandboxCommand(prompt: string, workDir = '/workspace', _schema?: object): string {
    ...
    const source = this.config.sandboxArgs ?? this.config.args ?? [];
    const args = this.resolveArgs(source, `'${escaped}'`, workDir);
    return `${this.config.command} ${args.join(' ')}`.trimEnd();
  }
  ...
  async extractLog(client: MicrosandboxClient): Promise<string | null> {
    const pattern = this.config.logPattern;
