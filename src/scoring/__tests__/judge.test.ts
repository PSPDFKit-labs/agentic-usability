...
import { formatSolution, runSandboxedJudge, buildJudgeAllowlist } from '../judge.js';
...
vi.mock('../../sandbox/microsandbox.js', () => ({
  MicrosandboxClient: vi.fn(function (this: any) {
...
vi.mock('../../sandbox/egress-logger.js', () => ({
  createEgressLockdownLogger: vi.fn().mockResolvedValue({
...
const TEST_SECRET = { value: '$ANTHROPIC_API_KEY' };
...
describe('runSandboxedJudge', () => {
  ...
  beforeEach(() => {
    ...
    mockClient.getSandbox.mockReturnValue({});
  ...
  it('returns a valid JudgeScore when sandbox agent returns valid JSON', async () => {
    ...
    const result = await runSandboxedJudge(
      makeTestCase(), [makeSolutionFile()], { command: 'claude', secret: TEST_SECRET },
      target, config, paths,
    );
  ...
  it('creates sandbox, scaffolds workspace, and shuts down on completion', async () => {
    const adapter = makeMockAdapter({ stdout: '' });
    mockCreateAdapter.mockReturnValue(adapter);
    mockClient.runCommandTimed.mockResolvedValue({
      exitCode: 0,
  ...
  it('throws when output is not valid JSON', async () => {
    ...
    await expect(
      runSandboxedJudge(makeTestCase(), [makeSolutionFile()], { command: 'claude', secret: TEST_SECRET }, target, config, paths),
  ...
  it('shuts down sandbox even when agent throws', async () => {
    const adapter = makeMockAdapter({ stdout: '' });
    mockCreateAdapter.mockReturnValue(adapter);
    mockClient.runCommandTimed.mockResolvedValue({
      exitCode: 0,
  ...
  it('uploads solution files when no snapshot is available', async () => {
    ...
    await runSandboxedJudge(
      ...
      { command: 'claude', secret: TEST_SECRET }, target, config, paths,
    );
  ...
  it('calls createEgressLockdownLogger with allowlist before running judge agent', async () => {
    const { createEgressLockdownLogger } = await import('../../sandbox/egress-logger.js') as any;
    ...
    await runSandboxedJudge(
      makeTestCase(), [makeSolutionFile()],
      { command: 'claude', secret: { envVar: 'ANTHROPIC_API_KEY', value: 'test-key', baseUrl: 'https://api.anthropic.com' } },
      target, config, paths,
    );

    expect(createEgressLockdownLogger).toHaveBeenCalledTimes(1);
    const allowedHosts: string[] = createEgressLockdownLogger.mock.calls[0][1];
...
describe('buildJudgeAllowlist', () => {
  it('includes agent secret baseUrl hostname', () => {
    const result = buildJudgeAllowlist(
      { command: 'claude', secret: { envVar: 'ANTHROPIC_API_KEY', value: 'key', baseUrl: 'https://api.anthropic.com' } },
      makeConfig(),
    );
    expect(result).toContain('api.anthropic.com');
  });

  it('includes secrets allowHosts', () => {
    const cfg = makeConfig({
      sandbox: {
        secrets: {
          API_KEY: { value: 'key', allowHosts: ['custom-api.example.com'], allowHostPatterns: [] },
        },
      },
    });
    const result = buildJudgeAllowlist({ command: 'claude', secret: TEST_SECRET }, cfg);
    expect(result).toContain('custom-api.example.com');
  });

  it('extracts hostnames from URL sources', () => {
    const cfg = makeConfig({
      publicInfo: [{ type: 'url', url: 'https://docs.example.com/api' }],
    });
    const result = buildJudgeAllowlist({ command: 'claude', secret: TEST_SECRET }, cfg);
    expect(result).toContain('docs.example.com');
  });

  it('extracts hostnames from git sources', () => {
    const cfg = makeConfig({
      privateInfo: [{ type: 'git', url: 'https://github.com/org/repo.git' }],
    });
    const result = buildJudgeAllowlist({ command: 'claude', secret: TEST_SECRET }, cfg);
    expect(result).toContain('github.com');
  });

  it('always includes hardcoded infra domains', () => {
    const result = buildJudgeAllowlist({ command: 'claude', secret: TEST_SECRET }, makeConfig());
    expect(result).toContain('github.com');
    expect(result).toContain('*.github.com');
  ...
  it('deduplicates hosts', () => {
    const cfg = makeConfig({
      sandbox: {
        secrets: {
          KEY: { value: 'k', allowHosts: ['github.com'], allowHostPatterns: [] },
        },
      },
    });
    const result = buildJudgeAllowlist({ command: 'claude', secret: TEST_SECRET }, cfg);
